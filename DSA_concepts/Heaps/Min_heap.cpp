...
public:
    ...
    int delete_heap_node()
    {
        ...
        while (index < size)
        {
            ...
            int min_index = index;
            if (left_index < size &&  arr[left_index] < arr[min_index])
            {
                min_index = left_index;
            }

            // check right child whether it is greater than new root
            if (right_index < size && arr[right_index] < arr[min_index])
            {
            ...
            else
            {
                ...
                index = min_index; // it will store next index to be checked
            }
        }
        return del_ele;
    }
...
{
    Heap h(20);
    h.insert_min_heap(10);
    h.insert_min_heap(20);
    h.insert_min_heap(40);
    h.insert_min_heap(90);
    h.insert_min_heap(60);
    h.insert_min_heap(30);
